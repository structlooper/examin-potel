...
<head>
  ...
  <link href="assets2/css/multiple_selection.css"  rel="stylesheet" type="text/css"questionText />
  <style>
    .questionText{
        margin-left: 3%;
      }
     ...
     <!-- <script type="text/javascript">
      ...
      $(function(){
        ...
        $(document).on("click", ".DropDownData", function(res){
          ...
          var uniqueIDs = [];
          var totalSelected;
        $(document).on("change", ".ItemsCheckBox", function(res){
          ...
          var isChecked = $(this).is(':checked');
            ...
            data1.push($(this).val());
            unique = $.unique(data1.sort());
            
            totalQuestionSelected = unique.length;            
            $('#totalQuestion').val(totalQuestionSelected);
            $('#update_totalQuestion').val(totalQuestionSelected)
            //id1.push($(this).attr("DropDownId"));
            uniqueIDs = $.unique(id1.sort());

          })                    
            }else if(isChecked==false){ //state false
              alert(isChecked);

              for(var i=0; i<unique.length; i++){
                var name = unique[i];
                if(name == $(this).val()){ unique.pop($(this).val());}
              }
              //alert("I am unchecked");
              //data.pop($(this).val());
              // if($.inArray($(this).val(),unique)){

              //   alert("Matched")
              //   //data.pop($(this).val());
              //  unique.splice( $.inArray($(this).val(),unique) ,1 );
              //  uniqueIDs.splice( $.inArray($(this).val(),uniqueIDs) ,1 );
              ...
              console.log("length is "+unique.length);
          }
            //alert(data);
          ...
          // $.each($(DivHead).find(".ItemsCheckBox:checked"), function(){
          //   data.push($(this).val());
          //   unique = $.unique(data.sort());
            
          //   totalQuestionSelected = unique.length;            
          //   $('#totalQuestion').val(totalQuestionSelected);
          //   $('#update_totalQuestion').val(totalQuestionSelected)
          //   id.push($(this).attr("DropDownId"));
          //   uniqueIDs = $.unique(id.sort());

          // })
          console.log("Lenth now"+unique.length);
          for(var i=0; i<=unique.length; i++){
            alert("len"+unique.length);
            datas1[i] = unique[i]+"";
            console.log("data is "+datas1[i]);
            //ids1[i] = uniqueIDs[i]+"";

            console.log("Id is "+ids1[i]);
          }


          FinalItem = datas1.toString();
          console.log("finalItmes "+FinalItem);         
          $(".DropDownId").val(FinalItem);
          //$(".showData").val(ids1);
 ...
 <script type="text/javascript">
      ...
      $(function(){
        ...
        $(document).on("click", ".ItemsCheckBox", function(res){
          ...
          let questionsIds = localStorage.getItem('questionsIds') ? JSON.parse(localStorage.getItem("questionsIds")): [];
          let ques = $(this).val()
          let quesId = $(this).attr('dropdownid')

           if (questions.includes(ques)) {
              questions.splice( $.inArray(ques,questions) ,1 );
              questionsIds.splice( $.inArray(quesId,questionsIds) ,1 );            
            }else{              
              questions.push(ques)
              questionsIds.push(quesId)
            }

          localStorage.setItem("questions", JSON.stringify(questions));
          localStorage.setItem("questionsIds", JSON.stringify(questionsIds));
          ...
          let appendquestions = JSON.parse(localStorage.getItem("questions"));  
          let appendquestionsId = JSON.parse(localStorage.getItem("questionsIds"));
          var totalQuestionSelected = appendquestionsId.length;
          $(".DropDownId").val(appendquestions.toString());
          $(".showData").val(appendquestionsId.toString());
          $('#totalQuestion').val(totalQuestionSelected);
          $('#update_totalQuestion').val(totalQuestionSelected)
  }) // Select CheckBox Close   
  ...
  <section class="content">
    <div class="container-fluid ">
      <div class="row ">
        ...
        <?php
$query_prev = "SELECT distinct(questions_ids) FROM exam";
$run_query = mysqli_query($conn, $query_prev);
//Count total number of rows
$count = mysqli_num_rows($run_query);
if($count > 0){
  while($exam_question_priv = mysqli_fetch_assoc($run_query)){ 
 $exam_quest[] = ($exam_question_priv);

}
}

$singleArray = []; 
foreach ($exam_quest as $questions_ids) 
{ 
  foreach ($questions_ids as $value) 
  { 
  $singleArray[] = $value; 
  } 
}

 $str = (implode(',', $singleArray));
 $uniqueStr = implode(',', array_unique(explode(',', $str)));
 echo '<input type="hidden" id="selected_questions" value="'.$uniqueStr.'" />'
?>
