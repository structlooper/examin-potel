...
if(isset($_GET['id']) && $_GET['id']!=''){   
  ...
  $query = "UPDATE blog_category SET status='$status' WHERE id ='$id'";
  $result = mysqli_query($conn,$query);
}
  ...
  <script>
    ...
    if(post_arr.length > 0){
      ...
      if (isDelete == true) {
           ...
           $.ajax({
            url: 'ajax/blog_category_ajax.php',
            type: 'POST',
            data: {categoryid: post_arr, tableName:"blog_category"},
...
<div class="content-wrapper ">
  ...
  <section class="content-header ">
    <div class="container-fluid ">
      <div class="row mb-2 ">
        <div class="col-sm-6 ">
          <a href= "#add_blogcatForm" button type="button " class="popup-with-form btn btn-primary btn-sm ">Add New Category</button></a>
    ...
    <div class="container-fluid ">
      <div class="row ">
          ...
          <table class="table ">
            ...
            <tbody>
              ...
              while($categoryList = mysqli_fetch_assoc($res)){ ?>
                <tr id='tr_<?=$categoryList['id'];?>'>
                  <th scope="row ">    

                    <!-- checkbox -->
                    <input type="checkbox" id='del_<?php echo $categoryList['id'];?>' name="delete[]" value="<?=$categoryList['id']; ?>" class="chk">
                  </th>
                  <td scope="row "><?=$i;?></td>
                  <td scope="row "><?=$categoryList['title'];?></td>
                  ...
                  <td>
                    <a href="#update_blog_category" button type="button" class="popup-with-form btn btn-primary btn-sm edit_blog_category" data-category_id="<?php echo $categoryList['id'];?>" data-package_name="<?php echo $categoryList['title'] ?>">Edit &nbsp;<i class='fa fa-edit'></i></button></a>

                  </a>
                </td>

                <td class="delete"><a href="?did=<?php echo $categoryList['id'];?>" data-id="<?php echo $categoryList['id'];?>" id="<?php echo $categoryList['id'];?>"> <button class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure ?')"> Delete &nbsp;<i class='fa fa-trash'></i></button></a></td>
       ...
       <!-- Start Add Class Form 
        ============================================= -->
        <form action="add_update_blog_category.php" method="post" id="add_blogcatForm" class="mfp-hide white-popup-block" enctype="multipart/form-data">
         <div class="col-md-12 login-custom">           
          <h4>Add Blog Category</h4>
          <div class="card-body">

          <div class="form-group row">
             <label for="colFormLabelSm" class="col-sm-12">Title</label>
            </div>
            <div class="form-group row">
            <div class="col-sm-12">
              <input type="text" class="form-control" id="title" name="title" placeholder="Enter Title" maxlength="100" autocomplete="off">
            </div>
          </div>
...
<form action="add_update_blog_category.php" method="post" id="update_blog_category" class="mfp-hide white-popup-block" enctype="multipart/form-data">
  <div class="col-md-12 login-custom">                                
    ...
    <div class="card-body">
      ...
      <div class="form-group row">
        <div class="col-sm-12">
          ...
          <input type="text" class="form-control" id="update_blogTitle" name="update_categoryTitle" maxlength="100" autocomplete="off" value="">
        </div>
      </div>  
    ...
    <div class="col-md-12">
      <div class="row">
        <input type="hidden" name="cat_id" id="blogcat_hidden_id">
...
<script type="text/javascript">
  $(document).ready(function(){
     ...
     $('.edit_blog_category').click(function() {
      var category_id = $(this).data('category_id');
      //console.log(category_id);
     // AJAX Request
     $.ajax({
      url: 'edit_blog_category.php',
      type: 'POST',
      data: {category_id: category_id},
      success: function(data){
          console.log('respo',data)
          //parse json data coming from edit_blog_category.php
          var blogCategoryData = JSON.parse(data);
          var cat_id = blogCategoryData[0].id ;

        //get the values of json array and store in jquery variables..
        var title = blogCategoryData[0].title;
        var description = blogCategoryData[0].description;

        //Now assign the values to the fields of modal
        $('#blogcat_hidden_id').val(cat_id);
        $('#update_blogTitle').val(title);
...
<?php }
 ...
 if(isset($_GET['msg']) && $_GET['msg']=="updated"){ ?>
   <script>
    ...
    swal({
      title: "<?php echo $_GET['msg']; ?>",
      text: "Blog category updated success!",
